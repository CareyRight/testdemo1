(function(){
var ui = angular.module('hypaas.ui');
...
ui.formInput('BpmnEditor', {
  ...
  controller: ['$scope', '$element', 'ActionService', function ($scope, $element, ActionService) {
    ...
    function onNodeEdit(element) {
      ...
      child.getContext = function() {
        return {
          _model: "com.hypaas.studio.db.WkfNode",
          xmlId: id,
          wkf: {
            id: $scope.record.id
          }
      ...
      var handler = ActionService.handler(child, $element, {
        action: "com.hypaas.studio.web.WkfController:onNodeEdit"
      });
    ...
    function onTransitionEdit(element) {
      ...
      child.getContext = function() {
        return {
          _model: "com.hypaas.studio.db.WkfTransition",
          xmlId: id,
          wkf: {
            id: $scope.record.id
          }
      ...
      var handler = ActionService.handler(child, $element, {
        action: "com.hypaas.studio.web.WkfController:onTransitionEdit"
      });
  ...
  link: function(scope, element, attrs, model) {
    var canvas = element.find('#js-canvas');
    var properties = element.find('#js-properties-panel');
    // var overrideModule = {
    ...
    //     }
    // });
      var BpmnModeler = window.BpmnJS.bpmnModeler;
      var camundaModdleDescriptor = window.BpmnJS.camundaModdleDescriptor;
    ...
    model.$parsers.unshift(function(value) {
      ...
      var invalidElements = _.filter(modeler._definitions.rootElements[0].flowElements, function(e) {
        return !e.name;
      });

      var elements = _.filter(modeler._definitions.rootElements[0].flowElements, function(e) {
        return e.$type != "bpmn:SequenceFlow" && e.$type != "bpmn:StartEvent";
      });

      var start = _.filter(modeler._definitions.rootElements[0].flowElements, function(e) {
  ...
  template:
    "<div>" +
      "<div class='canvas bpmn-canvas' id='js-canvas'></div>" +
