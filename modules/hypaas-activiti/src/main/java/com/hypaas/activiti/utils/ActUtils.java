...
public class ActUtils {
  ...
  @Inject private ProcessEngineConfiguration processEngineConfiguration;

  /**
   * 设置会签节点属性 会签相关变量注释：nrOfInstances：实例总数 nrOfActiveInstances：当前活动的，比如，还没完成的，实例数量。 对于顺序执行的多实例，值一直为1
   ...
   */
  public void setSequenceFlowCondition(String modelId, String nodelId, String condition)
      throws IOException {
    // 获取模型--设置连线条件 到 流程中
    ...
    if (historicProcessInstance.getEndTime() != null) { // 该流程已经结束
      ...
      inputStream =
          diagramGenerator.generateDiagram(
              bpmnModel, "png", activeActivityIds, highLightedFlows, "宋体", "宋体", null, null, 1.0);
    }
    return inputStream;
  }
   ...
   * @return
   */
  public List<PvmActivity> getNextActNodes(
      String defid, String nodelId, Map<String, Object> elMap) {
    List<PvmActivity> pvmActivities = new ArrayList<PvmActivity>();
    ProcessDefinition processDefinition =
        repositoryService.createProcessDefinitionQuery().processDefinitionId(defid).singleResult();
   ...
   * @return
   */
  public List<PvmActivity> getNextActivities(
      List<PvmTransition> pvmTransitions, Map<String, Object> elMap) {
    List<PvmActivity> pvmActivities = new ArrayList<PvmActivity>();
  ...
  public ActivityImpl findActivitiImpl(String taskId, String activityId) throws Exception {
    ...
    ActivityImpl activityImpl =
        ((ProcessDefinitionImpl) processDefinition).findActivity(activityId);
  ...
  public void backProcess(
      String taskId, String activityId, Map<String, Object> variables, String comment)
      throws Exception {
    ...
    for (ActivityImpl activityImpl : activitiList) {
      ...
      if (activitiId.equals(id)) {
        ...
        List<PvmTransition> outTransitions =
            activityImpl.getIncomingTransitions(); // 获取从某个节点出来的所有线路
   ...
   * @return
   */
  private List<PvmTransition> forCheckQueryPvm(
      List<PvmTransition> pvmTransitions, List<PvmTransition> pvmTransitionList) {
    for (PvmTransition pvmTransition : pvmTransitions) {
      ...
      if ("completed".equals(historicTaskInstance.getDeleteReason())) {
        List<PvmTransition> outgoingTransitions =
            pvmTransition.getDestination().getOutgoingTransitions();
   ...
   * @throws Exception
   */
  private void startParallelGateways(
      Map map,
      ProcessInstance processInstance,
   ...
   * @throws Exception
   */
  private void endParallelGateways(
      Map map,
      ProcessInstance processInstance,
      Map<String, Object> variables,
      List<String> list,
  ...
  private void commitProcess(
      String taskId, Map<String, Object> variables, List<String> activityIds, String comment)
      throws Exception {
    if (variables == null) {
      variables = new HashMap<String, Object>();
    }
  ...
  private void turnTransition(
      String taskId, List<String> activityIds, Map<String, Object> variables, String comment)
      throws Exception {
    // 当前节点
    ActivityImpl currActivity = findActivitiImpl(taskId, null);
    // 清空当前流向
  ...
  public void setBackTaskDealer(Task task, String activityId) {
    ...
    if (list != null && list.size() > 0) {
      ...
      if (taskList != null && taskList.size() > 1) {
        ...
        // 顺序会签流程变量处理人
        String variable =
            (String) runtimeService.getVariable(taskNew.getExecutionId(), "countersign");
  ...
  public String getProActivityId(
      String processDefinitionKey, String activitiId, String processInstanceId, String preNodeId) {
    List<PvmActivity> pvmActivities = new ArrayList<PvmActivity>();
    ProcessDefinition processDefinition =
        repositoryService
            .createProcessDefinitionQuery()
        ...
        if (activitiId.equals(id)) {
          ...
          for (PvmTransition tr : pvmTransitions) {
            ...
            if ("userTask".equals(type)) {
              ...
              PvmActivity exclusiveActiviti =
                  getExclusiveActiviti(ac, processDefinitionKey, processInstanceId);
              ...
              // 其它情况则进行递归 并行路由
              preNodeId =
                  getProActivityId(processDefinitionKey, ac.getId(), processInstanceId, preNodeId);
            }
          }
        }
      }
   ...
   * @return
   */
  public PvmActivity getExclusiveActiviti(
      PvmActivity ac, String processDefinitionKey, String processInstanceId) {
