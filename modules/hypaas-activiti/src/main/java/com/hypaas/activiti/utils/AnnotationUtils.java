...
/** 类的功能描述. @Auther hxy @Date 2017/8/11 */
public class AnnotationUtils {

  /**
   * 通过类路径,通过反射获取类属性上的ActField注解信息,在获取属性时， 最多支持向上获取两层父类属性,如需要可自行添加
   *
   * @param classurl
        ...
        if (sourcePd != null && sourcePd.getWriteMethod() != null) {
          try {
            ...
            try {
              ...
              try {
                actField =
                    clazz.getSuperclass().getDeclaredField(keyName).getAnnotation(ActField.class);
              } catch (NoSuchFieldException el) {
                actField =
                    clazz
                        .getSuperclass()
                        .getSuperclass()
                        .getDeclaredField(keyName)
                        .getAnnotation(ActField.class);
              }
            }
            if (actField != null) {
